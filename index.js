addEventListener('fetch', event => {
  ...
  <html lang="en">
      <head>
          <title>G2FS (Grafana 2 Feishu(Lark)) Serverless Edition</title>
      ...
      <body class="antialiased font-sans">
          <div class="md:flex min-h-screen" style="display: flex;">
              <div class="w-full md:w-1/3 bg-white flex items-center justify-center">
                  <div class="max-w-sm m-8">
                      <div class="text-black text-5xl md:text-7xl font-black">
                      ...
                      <ol>
                          ...
                          <li>After creation, a "Webhook URL" will display, if it doesn't, just click the robot icon and it will display, copy key from <pre>https://open.feishu.cn/open-apis/bot/v2/hook/YOUR_KEY</pre></li>
                          <li>In Grafana, Create a Webhook notification, Paste url with: <pre>https://g2fs.knat.network/YOUR_KEY</pre>.</li>
                      </ol>
...
async function postFeiShuUrl(request) {
  const key = request.url.replace(/^https:\/\/.*?\//gi, "")

  const feishu_webhook_url = "https://open.feishu.cn/open-apis/bot/v2/hook/" + key


  const json_obj = await request.json()

  const msg_content = "**" + json_obj['title'] + "**" + "\n" + json_obj['message']
  ...
  const response = await fetch(feishu_webhook_url, init)
